...
def main():
    ...
    if args.filter == "native":
        ...
        sentences, log_marginal, min_ess = run_smc_substitution(
            key, jax.numpy.array(obs), num_particles=args.particles, max_dist=args.max_dist,
            max_deletions=args.max_deletions, allow_insertion=not args.no_insertion, progress=True,
        )
    else:
