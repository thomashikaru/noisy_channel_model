...
Run:  python -m genjax_port.poc_word_indel
"""
...
from genjax_port.poc_word_smc import (VOCAB, V, VOCAB_IDS, VOCAB_LEN, WORD2IDX,
                                      CORPUS, LCTX)
...
def _word_row_update(log_alpha, emit_col, wdel, wins):
    ...
    def ins_step(left, b):                     # spurious observed word (insert): left-to-right sweep
        cell = logsumexp(jnp.stack([b, left + wins]))
        return cell, cell

    _, rest = jax.lax.scan(ins_step, beta[0], beta[1:])
...
def _wins_only_row(log_alpha, wins):
    ...
    ``new_alpha[k+1] = log_alpha[k] + wins``; ``new_alpha[0] = -inf`` (must advance consumption)."""
    return jnp.concatenate([jnp.array([-jnp.inf], dtype=log_alpha.dtype), log_alpha[:-1] + wins])
