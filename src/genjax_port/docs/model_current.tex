...
channel (the \emph{word-action} channel of \S\ref{sec:word-action}). Comprehension/correction is the
...
\code{channel} argument with two values (\code{pythia\_word\_caprop.run}):
\code{"word\_action"} is the model just described; \code{"char\_copy"} is the original bundled
character channel of \S\ref{sec:char-channel}--\S\ref{sec:word-channel}, retained only as the
exact-enumeration certification anchor (\S\ref{sec:certification}). The two are not independent
codepaths: \code{char\_copy} is exactly the \emph{zero-action degenerate} of \eqref{eq:wa-emit}, with
...
correction---so $(200,1,1,1)$ is the deployed \code{ACTION\_ALPHA\_DEFAULT}. Two effects the
prior cannot fix---and must not be chased with $\alpha$---are the language model's own
...
\midrule
\multicolumn{4}{@{}l}{\emph{word-action channel} (\S\ref{sec:word-action}; supersedes $\wdel,\rho_{\insact}$)} \\
...
\code{channel} & noise-model selector & \code{word\_action}\,/\,\code{char\_copy} & \code{channel} \\
\bottomrule
\end{tabular}
\end{center}
...
The action prior's default $(200,1,1,1)$ is the value the \S\ref{sec:calibration} sweep selected; with it
in place, \code{word\_action} is the default \code{channel} (\code{char\_copy} remaining only as the
