...
not update it during the filtering sweep; this is a deliberate approximation (a parameter rejuvenation
...
\end{equation}
identical in form to the filtering-sweep proposal \eqref{eq:proposal} but evaluated against the
\emph{current} (possibly later-updated) context. Because $\mathcal B$ has fixed cardinality, the
overwrite is dimension-preserving and Theorem~\ref{thm:mh}/\ref{thm:rejuv-smcp3} apply directly.

This move is implemented in \code{rejuvenation.py} on an unrolled per-word chain model (addresses
$x_0,\dots,x_{W-1}$, $o_0,\dots,o_{W-1}$, with the LM buffer threaded). Editing $x_k$ triggers the
\code{Update} to re-score the \emph{suffix} $x_{k+1{:}W}$ (their $\pLM$ depends on the changed
buffer), so the weight $w_{\mathrm{upd}}$ in \eqref{eq:wupd} carries the later-context vote --
\S\ref{sec:extensions}'s reach/cost discussion. (Implementation note: a \code{Rejuvenate}
\code{argument\_mapping} receives only the \emph{local} sub-trace at the edited address, not the
full trace, so the position-$k$ context is reconstructed from the full trace and closed over.)
Validation (\code{tests/test\_rejuvenation.py}, pythia-70m): a literal ``he wants \textsf{too} go
home'' is corrected \textsf{too}$\to$\textsf{to} by a flip sweep; the flip's MH weight is $\approx 0$
without the trailing words but $\approx +5.5$ with them (the suffix vote); and on a 2-word toy the MH
stationary histogram matches the brute-force exact posterior to $\le 10^{-3}$ -- Theorem~\ref{thm:mh}
in practice. The earlier single-address spike (\code{tests/test\_noisy\_channel.py}) flips a literal
$x=\textsf{`` too''}$ to $\textsf{`` to''}$ under \eqref{eq:r1prop}.
...
This move is implemented in \code{rejuvenation\_r2.py} on a \emph{masked deletion-gap} chain
(addresses $\code{del}_t,\ \code{gap}_t/\code{xd},\ x_t,\ o_t$): each gap is a
$\code{mask}(\pLM)$ slot whose boolean $\code{del}_t$ is the trans-dimensional coordinate, so the
move is a \emph{fixed-address} \code{Update} (the address set never changes — variable length is
absorbed into the mask). Crucially genjax's \code{MaskCombinator} supplies the reversible-jump
birth/death weight for free: flipping $\code{del}_k$ \textsf{false}$\to$\textsf{true} reports
$w_{\mathrm{upd}}$ equal to the born slot's full score (Bernoulli del-prior $+\ \pLM(\text{omitted})\ +$
the automatic suffix re-score), and \textsf{true}$\to$\textsf{false} reports its negation, round-tripping
exactly. Unlike R1 the move does \emph{not} use a single-proposal \code{Rejuvenate} (the forward add
draws a token, the backward delete draws none — asymmetric supports); the SMCP3 weight
\eqref{eq:smcp3} is assembled directly, with $Q$ the LM-lookahead omitted-token proposal of
\eqref{eq:proposal}. Validation (\code{tests/test\_rejuvenation\_r2.py}, pythia-70m): a literal
``he wants \_\ go home'' (the word ``to'' omitted) is repaired ``to''-inserted by an add sweep;
the add weight grows with the disambiguating suffix; and on a one-gap toy the MH stationary histogram
matches the enumerated exact posterior over the reachable states to $\le 10^{-2}$ — Theorem~\ref{thm:mh}
for the trans-dimensional move. (Vectorization over particles is future work, \code{R2\_PLAN.md}.)
...
The filter augments each step with two further operations. Deletion is implemented in the native
port (\code{smc\_substitution.run\_smc\_substitution}, \code{max\_deletions}${>}0$; the M2
milestone) and both are present in the reference filter (\code{particle\_filter\_unified.py}). The
SMC and rejuvenation theory are unchanged: only the branch set and the per-step weight gain extra,
\emph{properly weighted}, factors.

\paragraph{Insertion ($\inssact$).} The observed word is spurious: $\mathrm{emit}(\inssact)=()$
(it consumes $w_i$ and emits no intended token) and $\log P(w_i\given\inssact) = n_i\cdot(-\log V)$,
a flat per-token cost. It adds one column to $E_i$ with branch evidence
$\log\pi_{\inssact} + n_i(-\log V)$, so Lemma~\ref{lem:collapse} and Theorem~\ref{thm:smc} apply
verbatim to the augmented set $\mathcal A_i\cup\{\inssact\}$.

\paragraph{Deletion ($\delact$): a properly weighted pre-extension.} Before explaining $w_i$, a
particle may posit up to $\Delta$ (=\code{MAX\_DELETIONS}) omitted single-token intended words. Each
slot is an auxiliary SIS extension that prepends one intended token $x$ to $z_{<i}$. A Bernoulli
``delete?'' choice is \emph{proposed} at rate $q_\delta = \code{P\_DELETE\_PROPOSAL}$ but
\emph{priced} at the model rate $p_\delta=\code{P\_DELETE\_PRIOR}$; conditioned on deleting, $x$ is
proposed from the top-$\kappa$ LM tokens reweighted by a one-step \emph{lookahead} toward the word's
first token $o_{i,1}$:
\begin{equation}\label{eq:del-prop}
  q_\delta(x\given z_{<i}) \;\propto\; \pLM(x\given z_{<i})\,\pLM(o_{i,1}\given z_{<i}\Vert x),
  \qquad x\in\mathrm{top}\text{-}\kappa\,\pLM(\cdot\given z_{<i}).
\end{equation}
The target contribution of a posited deletion is just its LM prior $\pLM(x\given z_{<i})$ (the
lookahead factor is a proposal device only). The resulting incremental gap weight is, per slot,
\begin{equation}\label{eq:gapweight}
  \log w^{\mathrm{gap}} =
  \begin{cases}
    \underbrace{\log\tfrac{p_\delta}{q_\delta}}_{\code{log\_del}}
      \;+\; \underbrace{\log\!\!\sum_{x'}\! e^{\,\ell(x')} - \ell(x)}_{\text{token IS correction}}
      & \text{delete }x,\\[1.2ex]
    \underbrace{\log\tfrac{1-p_\delta}{1-q_\delta}}_{\code{log\_keep}} & \text{stop},
  \end{cases}
  \qquad \ell(x') = \log\pLM(x'\given z_{<i}) + \log\pLM(o_{i,1}\given z_{<i}\Vert x'),
\end{equation}
matching \code{deletion\_gap} in \code{smc\_substitution.py}. The token correction is exactly
$\log\big[\pLM(x\given z_{<i})/q_\delta(x)\big]$ (the lookahead $\ell$-terms cancel against
\eqref{eq:del-prop} up to its normalizer), so the slot is a valid SIS step: it multiplies a
properly weighted factor into the particle weight and Theorem~\ref{thm:smc} extends to the
gap-augmented sweep. The deletion is applied \emph{before} $E_i$ so the posited token enters the LM
context of the word that follows.

\paragraph{Native trace carrier.} In the \code{@gen} model these are additional \code{Switch}
branches (insertion: a zero-token branch; deletion: a masked variable-length pre-emission via the
\code{Mask} combinator). The filtering sweep computes the weights directly, as above; the proofs are
unchanged because they use only the branch evidences $E_i$, the gap weight \eqref{eq:gapweight}, and
the \GFI{} identities.
